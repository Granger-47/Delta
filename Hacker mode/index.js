...
function place(num){
    movehistory.push([null,num,turn,outer,middle,inner,middleunlocked,innerunlocked]);
    placementsound();
    
    if (outer===6 && !middleunlocked){middleunlocked=true;
        document.getElementById("commands").insertAdjacentHTML(`afterbegin`,`<div class="element">
            <div class="undoredo">Middle circuit is now unlocked</div>
        </div>`);
    }
    ...
    if (turn==="red"&&reds!==0){
        ...
        toggle();
        updatehistory();
        checkforelimination(num,"red");
        undohistory = [];
        turn = "blue";
        occupied[num] = true;
        reds-=1;
    }
    else if (turn==="blue"&&blues!==0){
        document.getElementById(`circle${String(num)}`).style.backgroundColor = "blue";
        toggle();
        updatehistory();
        checkforelimination(num,"blue");
        undohistory = [];
        turn = "red";
        occupied[num] = true;
        blues-=1;
...
function placementstage(callback,num){
    if (!paused){
        if (!middleunlocked){
            ...
            if ((reds!==0 || blues!==0)&&!occupied[num]){
                if(num<12){
                    middle += 1;
                    place(num);
                    document.getElementById("player").textContent = turn.charAt(0).toUpperCase() + turn.slice(1);
...
function move(num1, num2) {
    if (!paused && adjacency[num1][num2] === 1 && num1 === active && !occupied[num2]) {
        
        if (getLayer(num2) === "inner" && !innerunlocked) return;

        movementsound();

        movehistory.push([num1, num2, turn, outer, middle, inner, middleunlocked, innerunlocked]);

        document.getElementById(`circle${num1}`).style.backgroundColor = "";
        document.getElementById(`circle${num2}`).style.backgroundColor = turn;
        ...
        if (middle === 6 && !innerunlocked) {
            innerunlocked = true;
            document.getElementById("commands").insertAdjacentHTML(`afterbegin`,
                `<div class="element"><div class="undoredo">Inner circuit is now unlocked</div></div>`
            );
        }

        toggle();
        updatehistory();
        checkforelimination(num2, turn);
